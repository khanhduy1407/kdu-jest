...
module.exports = function compileTypescript (scriptContent, kduJestConfig, filePath) {
  ensureRequire('typescript', ['typescript'])
  const typescript = require('typescript')
  const tsConfig = loadTypescriptConfig(kduJestConfig)
  ...
  if (tsConfig.compilerOptions.module !== 'commonjs' && !loadBabelConfig(kduJestConfig, filePath)) {
    inlineBabelConfig = {
      plugins: [
  ...
  return compileBabel(res.outputText, inputSourceMap, inlineBabelConfig, kduJestConfig)
}
