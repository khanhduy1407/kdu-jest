...
module.exports = (content, filePath, jestConfig = {}) => {
  ...
  let scssResources = ''
  if (kduJestConfig.resources && kduJestConfig.resources.scss) {
    scssResources = kduJestConfig.resources.scss
  ...
  try {
    ...
    if (!kduJestConfig.hideStyleWarn) {
      logger.warn(`There was an error rendering the SCSS in ${filePath}. SCSS is fully supported by kdu-jest. Still some features might throw errors. Webpack aliases are a common cause of errors. If you use Webpack aliases, please use jest's suggested way via moduleNameMapper which is supported.`)
